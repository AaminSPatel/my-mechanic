...
export default function InvoiceContent() {
  ...
  const handleDownload = async () => {
    if (!previewRef.current || !formData.serviceName || downloading) return;
    setDownloading(true);
    ...
    try {
      ...
      // 1. Download file to admin/user device
      const link = document.createElement('a');
      link.download = `${formData.customerName || 'customer'}-invoice.png`;
      link.href = dataUrl;
      link.click();

      // 2. Send email notification with invoice details & attachment to Admin
      try {
        ...
        console.error('Invoice email dispatch failed:', apiErr);
        setEmailStatus('error');
      }
    } catch (err) {
  ...
  const company = {
    ...
    address: 'Nayta Mundla Main Road, Near Palda & Tejaji Nagar, Indore, MP',
    ...
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '1rem', fontFamily: 'sans-serif', backgroundColor: '#f8fafc' }}>
      <header style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: '800', color: '#1e293b' }}>
          MyMechanic24 Invoice Generator
        </h1>
        <p style={{ fontSize: '0.85rem', color: '#64748b', marginTop: '4px' }}>
          Fill details below. On download, a copy is automatically emailed to admin.
        </p>
      </header>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', alignItems: 'center' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={inputWrapperStyle}>
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="Full Service, Oil Change, Doorstep Wash, etc."
                style={inputStyle}
                required
              />
            </div>
          ...
          <button
            ...
            disabled={!formData.serviceName || downloading}
            style={{
              width: '100%',
              marginTop: '2rem',
              padding: '1rem',
              backgroundColor: !formData.serviceName
                ? '#94a3b8'
                : downloading
                ? '#1d4ed8'
                : '#2563eb',
              ...
              fontWeight: '700',
              fontSize: '1rem',
              cursor: formData.serviceName && !downloading ? 'pointer' : 'not-allowed',
              boxShadow: '0 4px 6px -1px rgba(37, 99, 235, 0.2)',
              transition: 'background-color 0.2s',
            }}
          >
            {downloading ? 'Generating & Emailing Admin...' : 'Download Invoice PNG & Email Admin'}
          </button>
          ...
          {emailStatus === 'sent' && (
            ...
            >
              ✅ Invoice downloaded & copy emailed to admin!
            </div>
          )}
          {emailStatus === 'error' && (
            <div
            ...
            >
              ⚠️ Invoice downloaded, but could not deliver email copy. Please check your credentials.
            </div>
          )}
        </div>
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <p style={{ fontSize: '0.65rem', color: '#64748b', margin: 0 }}>
                  Authorized Car Service Center & Doorstep Care
                </p>
              </div>
            </div>
            ...
            >
              <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '8px' }}>
                <div>
                  <p style={{ fontSize: '0.6rem', color: '#94a3b8', margin: 0, fontWeight: '700' }}>
                    CUSTOMER
                  </p>
                  <p style={{ fontSize: '0.8rem', fontWeight: '700', color: '#1e293b', margin: '2px 0' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <p style={{ fontSize: '0.8rem', fontWeight: '700', color: '#1e293b', margin: '2px 0' }}>
                    {formData.brand} {formData.carModel}
                  </p>
                  <p style={{ fontSize: '0.7rem', color: '#64748b', margin: 0 }}>
            ...
            >
              <span>
                INV: <b style={{ color: '#1e293b' }}>#INV-{formData.date?.replace(/-/g, '')}</b>
              </span>
              <span>
                KM: <b style={{ color: '#1e293b' }}>{formData.odometer || '---'}</b>
              </span>
              <span>
                DATE: <b style={{ color: '#1e293b' }}>{formData.date}</b>
              </span>
            </div>
            ...
            <div style={{ textAlign: 'center', borderTop: '1px dashed #cbd5e1', paddingTop: '1rem' }}>
              <p style={{ fontSize: '0.8rem', fontWeight: '800', color: '#2563eb', margin: '0 0 4px 0' }}>
                THANKS FOR YOUR VISIT!
              </p>
              <p style={{ fontSize: '0.6rem', color: '#64748b', margin: '2px 0' }}>
                {company.address}
              </p>
              <p style={{ fontSize: '0.6rem', color: '#64748b', margin: '2px 0' }}>
                {company.phone}
              </p>
              <p style={{ fontSize: '0.5rem', color: '#94a3b8', marginTop: '10px' }}>
                Powered by MyMechanic24
              </p>
            </div>
          </div>
        </div>
